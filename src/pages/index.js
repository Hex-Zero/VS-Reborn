    ...
    <Layout location={location} title={siteTitle}>
      ...
      <section className="max-main-image-wrapper">
        {data.allFile.edges.map((element, index) => {
          ...
          const layout = returnLayoutSize(
            sharpChild.original.width,
            sharpChild.original.height
          );
          return (
            <div
              key={index}
              onClick={(e) => setImageIndex({ event: e, index: index + 1 })}
              className={"max-main-image " + layout}
            >
              <Img fluid={sharpChild.fluid} />
            </div>
          );
        })}
      </section>
